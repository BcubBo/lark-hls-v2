...
# ---------------------------------------------------------------------------
# User cache: SQLite-based open_id → name mapping with role permissions
# ---------------------------------------------------------------------------
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Initialize user cache for external users
        self._user_cache = FeishuUserCache(str(get_hermes_home() / "feishu.group.sqlite3"))
    ...
    async def _handle_reaction_event(self, event_type: str, data: Any) -> None:
        ...
        source = self.build_source(
            ...
            role_authorized=sender_profile.get("system_role") in ("admin", "moderator"),
        )
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        For external users (not in app visibility), falls back to SQLite cache.
        """
        ...
        # Auto-cache successful API lookups (cache with both open_id and user_id if available)
        if display_name:
            if name_lookup_id:
                self._user_cache.set_user(name_lookup_id, display_name, source="api", chat_id=chat_id)
            if open_id and open_id != name_lookup_id:
                self._user_cache.set_user(open_id, display_name, source="api", chat_id=chat_id)
                # 关联open_id和user_id
                self._user_cache.link_ids(open_id, name_lookup_id)
        
        # Fallback: SQLite cache for external users (check open_id first, then name_lookup_id)
        if not display_name:
            if open_id:
                display_name = self._user_cache.get_name(open_id)
            if not display_name and name_lookup_id:
                display_name = self._user_cache.get_name(name_lookup_id)
        
        # Query system_role from SQLite
        system_role = "member"
        lookup_ids = [i for i in [open_id, name_lookup_id] if i]
        for lid in lookup_ids:
            user_info = self._user_cache.get_user(lid)
            if user_info:
